...
<<<<<<< HEAD
```{r, fig.cap="Model graph without background to better view data  points"}
...
>>>>>>> d1e718f72161c15cb0a26275523380285239557f
ggplot(makeup_shades, aes(lightness, price, col = best_seller)) +
  scale_x_continuous(breaks=c(0,20,40,60,80,100))+
  theme_minimal(base_size=20)+ 
  labs(title="Parallel Slopes Model of Makeup Data",
...
```{r constant variance plot, fig.cap= "**Figure 3.a** Residuals versus fitted values plot. Dotted line represents where residuals equal 0."}
...
What can't we trust as much: p-values, area under the curve, so can we really trust the model we chose based on the p-values, assuming each makeup is independent.
